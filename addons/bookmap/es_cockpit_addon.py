# ES Cockpit Bookmap Addon v1.0 - sweeps + delta divergence + tick capture
#
# Full-fidelity ES capture, identical logic to cockpit_addon.py (NQ).
# Connects to aggregator as source=bookmap-es to avoid connection-state
# conflict with the NQ addon. Load this in the ES Bookmap instance.
...
DELTA_PEAK_SWING_TICKS = 2.0              # min swing to confirm a peak/trough (1 tick = 0.25)
...
class WSSender(threading.Thread):
    def __init__(self, q, url=AGGREGATOR_URL, name="es-ws-sender"):
    ...
    def _connect(self):
        try:
            ...
            self.connected = True
            print("[es-cockpit] connected to " + self.url)
            return True
        except Exception as e:
            self.connected = False
            print("[es-cockpit] connect failed for " + self.url + ": " + str(e))
            return False
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                print("[es-cockpit] send failed: " + str(e) + "; will reconnect")
                self._close()
                try:
...
class TickBuffer:
    def __init__(self, send_queue, max_events=TICK_BATCH_MAX_EVENTS, flush_ms=TICK_BATCH_FLUSH_MS):
        ...
        self.stop_event = threading.Event()
        self._flusher = threading.Thread(target=self._run, daemon=True, name="es-tick-flusher")
    ...
    def _flush(self):
        ...
        try:
            ...
            print("[es-cockpit] tick send queue full, dropped batch of " + str(len(events)))
...
_tick_sender = WSSender(_tick_queue, url=TICK_STORE_URL, name="es-tick-sender")
...
def handle_subscribe_instrument(addon, alias, full_name, is_crypto, pips,
    ...
    try:
        ...
        if symbol is None:
            print("[es-cockpit] ignoring unsupported alias: " + str(alias))
            return

        with _lock:
            _symbols[alias] = SymbolState(alias, symbol, pips, size_multiplier)

        print("[es-cockpit] subscribed: alias=" + str(alias)
              + " symbol=" + str(symbol)
              + " pips=" + str(pips)
        ...
        print("[es-cockpit] handle_subscribe_instrument crashed: " + traceback.format_exc())
    ...
    try:
        with _lock:
            ...
            if state is not None:
                _evaluate_burst_unlocked(state)
                _emit_bar(state, False)
        print("[es-cockpit] detached: alias=" + str(alias))
    except Exception:
        print("[es-cockpit] handle_detach_instrument crashed: " + traceback.format_exc())
...
def on_trade(addon, alias, price, size, is_otc, is_bid_aggressor,
    ...
    try:
        ...
        print("[es-cockpit] on_trade crashed: " + traceback.format_exc())
...
def on_depth(addon, alias, is_bid, price_level, size_level):
    try:
        ...
        print("[es-cockpit] on_depth crashed: " + traceback.format_exc())
...
def on_interval(addon, alias):
    ...
    try:
        ...
        print("[es-cockpit] on_interval crashed: " + traceback.format_exc())
...
def main():
    print("[es-cockpit] addon starting (v1.0 - sweeps + delta divergence + tick capture)")
    ...
    _sender.start()
    print("[es-cockpit] sender thread started → " + AGGREGATOR_URL)

    _tick_sender.start()
    _tick_buffer.start()
    print("[es-cockpit] tick stream started → " + TICK_STORE_URL)
