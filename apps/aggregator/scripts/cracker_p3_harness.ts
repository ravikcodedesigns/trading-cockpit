...
export interface VisitRow {
  day: string; closeTs: number; openTs: number; levelId: string; source: string; kind: string; side: string; held: number;
  levelPrice: number; visitIndex: number; confluenceN: number | null; band: number; sigmaEv: number;
...
export function loadVisits(sym: string): { rows: VisitRow[]; days: string[] } {
  ...
  const raw = db.prepare(`
    SELECT vf.trading_day day, vf.close_ts closeTs, vf.open_ts openTs, vf.level_id levelId, vf.source, vf.kind, vf.side, vf.held,
      vf.level_price levelPrice, vf.visit_index visitIndex, vf.confluence_n confluenceN, vf.band, vf.sigma_ev sigmaEv,
