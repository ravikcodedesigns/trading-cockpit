...
const INSTR: Record<string, {
  ...
  NQ: { round: { step: 50, major: 100 }, sane: [20000, 40000], lm: {}, tr: {}, vp: {}, ms: {}, sg: {}, bs: {} },
  ES: {
    ...
    sg: { CAP_PT: 15 },
    bs: { K_WALL_TICKS: 4, W_BEYOND_TICKS: 10 },   // 16/40 NQ ticks ÷ 4 (price-scale)
  },
};
...
async function runDay(con: any, days: string[], di: number) {
  ...
  while ((chunk = await stream.fetchChunk()) && chunk.rowCount > 0) {
    for (const row of chunk.getRows() as any[]) {
      ...
      if (row[1] === 'D') { book.applyDepth({ ts, priceInt: book.intFromPrice(price), size, isBid: Number(row[4]) === 0 }); continue; }
