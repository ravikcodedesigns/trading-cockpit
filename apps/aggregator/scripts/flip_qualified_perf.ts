// Performance report: clean-impulse FLIP signals that passed the quality gate.
// INNER JOINs signals with qualified_signals — only signals the live system
// would have actually broadcast (gold-tier).
//
// TP/SL per current config (`clean-impulse-FLIP`):
//   TP = 80 pts (both)
//   SL = 55 pts (long) / 105 pts (short)
//
// 120-min forward window in ticks.db.
...
const rows = db.prepare(`
  ...
  ORDER BY s.ts ASC
`).all() as Array<{ id: number; ts: number; symbol: string; direction: 'long'|'short'; score: number; entry: number; ctx_gm: string|null; gate_reason: string; session: string }>;

console.log(`clean-impulse FLIP — QUALIFIED only: ${rows.length} signals\n`);
...
for (const s of rows) {
  ...
  results.push({ id: s.id, ts: s.ts, sym: s.symbol, dir: s.direction, score: s.score, entry: s.entry, outcome, pts, gm: s.ctx_gm, session: s.session });
}
...
console.log('\n═══ By session ═══');
summary(results.filter(r => r.session === 'rth'),       '  RTH');
summary(results.filter(r => r.session === 'overnight'), '  Overnight');
...
console.log('\n═══ Last 30 qualified (most recent first) ═══');
