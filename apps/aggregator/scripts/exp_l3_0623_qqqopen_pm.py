...
WIN_LO=et(9.5); WIN_HI=et(14.67)   # extended to capture the genuine ~14:24 QQQ Open touch
...
while True:
    ...
    for ts,src,oid,p,sz,bid,px in rows:
        ...
        if src=="DEP":
            ...
            if cpx is not None and WIN_LO<=ts<=WIN_HI and last_px is not None:
            ...
            if cpx is not None: last_px=cpx
...
print(f"{len(touches)} raw touches -> {len(keep)} visits, 09:30-14:40\n")
