...
const signals = db.prepare(`
  ...
  FROM signals
  WHERE rule_id = 'clean-impulse' AND direction = 'long'
  ORDER BY ts ASC
`).all() as any[];

console.log(`Total clean flip LONG signals: ${signals.length}`);

// Prepared query: get all ticks in a window for a symbol
...
for (const sig of signals) {
  ...
  let maxDD = 0;
  let hitTP = false;
  let tpIdx = fwd.length;

  for (let i = 0; i < fwd.length; i++) {
    if (fwd[i].price - entry >= WIN_PTS) { hitTP = true; tpIdx = i; break; }
  }

  // max_dd only up to when TP is hit
  for (let i = 0; i <= (hitTP ? tpIdx : fwd.length - 1); i++) {
    const dd = entry - fwd[i].price;
    if (dd > maxDD) maxDD = dd;
  }

  // max_gain in full 120min window
  for (const t of fwd) {
...
console.log('=== OVERALL ===');
stats(results, 'All clean flip LONG');
console.log();

console.log('=== BY SYMBOL ===');
for (const sym of ['NQ', 'ES']) stats(results.filter(x => x.sym === sym), sym);
console.log();

console.log('=== BY SCORE ===');
stats(results.filter(x => x.score === 100), 'Score 100');
stats(results.filter(x => x.score >= 90 && x.score < 100), 'Score 90-99');
stats(results.filter(x => x.score >= 80 && x.score < 90), 'Score 80-89');
stats(results.filter(x => x.score < 80), 'Score <80');
console.log();
...
console.log('=== STRATEGY CANDIDATES ===');
stats(results.filter(x => x.score === 100 && x.etMin >= 594 && x.sym === 'NQ'), 'NQ, Score=100, excl open');
stats(results.filter(x => x.score === 100 && x.etMin >= 594), 'All, Score=100, excl open');
stats(results.filter(x => x.score >= 90 && x.etMin >= 594 && x.sym === 'NQ'), 'NQ, Score>=90, excl open');
stats(results.filter(x => x.score >= 90 && x.etMin >= 594), 'All, Score>=90, excl open');
console.log();

console.log('=== ALL SIGNALS ===');
console.log(`${'Date'.padEnd(6)} ${'DOW'.padEnd(4)} ${'Sym'.padEnd(3)} ${'Time'.padEnd(6)} ${'Scr'.padEnd(4)} ${'Entry'.padStart(8)} ${'MaxGain'.padStart(8)} ${'MaxDD'.padStart(7)}  Out`);
console.log('-'.repeat(68));
for (const r of results) {
  const hh = Math.floor(r.etMin / 60).toString().padStart(2, '0');
  const mm = (r.etMin % 60).toString().padStart(2, '0');
  console.log(`${r.date.padEnd(6)} ${r.dow.padEnd(4)} ${r.sym.padEnd(3)} ${(hh + ':' + mm).padEnd(6)} ${String(r.score).padEnd(4)} ${r.entry.toFixed(2).padStart(8)} ${('+' + r.maxGain.toFixed(1)).padStart(8)} ${('-' + r.maxDD.toFixed(1)).padStart(7)}  ${r.outcome}`);
}
