...
const NEAR_TICKS = Number(process.env.NEAR_TICKS ?? 16);   // snapshot when price within ±N ticks of a level
...
function health(st: SymState): void {
  ...
  console.log(
    ...
    ` snaps=${st.snaps} log=${st.logPath ? path.basename(st.logPath) : 'none'}`,
  );
}

// ── main ─────────────────────────────────────────────────────────────────────
const states: SymState[] = [
  { sym: 'NQ', suffix: 'NQU6', book: new OrderBook('NQ'), tail: null, logPath: null, levels: [], lastSnap: new Map(), snaps: 0 },
  { sym: 'ES', suffix: 'ESU6', book: new OrderBook('ES'), tail: null, logPath: null, levels: [], lastSnap: new Map(), snaps: 0 },
];
